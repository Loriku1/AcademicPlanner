...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, Modal, TextInput, Button, Alert, ActivityIndicator } from 'react-native';
...
const GradeCalculator = () => {
  ...
  const [currentCourse, setCurrentCourse] = useState({ id: null, name: '', components: [] });
  const [currentComponent, setCurrentComponent] = useState({ name: '', weight: '', score: '' });
  ...
  const handleSaveCourse = () => {
    if (!currentCourse.name) return Alert.alert('Error', 'Course name is required');
    const newCourse = { ...currentCourse, id: currentCourse.id || Date.now().toString() };
    const updatedCourses = currentCourse.id ? courses.map(c => c.id === currentCourse.id ? newCourse : c) : [...courses, newCourse];
  ...
  const handleSaveComponent = () => {
    if (!currentComponent.name || !currentComponent.weight || isNaN(currentComponent.weight)) {
      return Alert.alert('Error', 'Please enter a valid component name and weight');
    }

    const updatedCourse = {
      ...currentCourse,
      components: [...currentCourse.components, { ...currentComponent, id: Date.now().toString() }]
    };
        ...
        renderItem={({ item }) => (
          <View style={styles.courseCard}>
            ...
            <View style={styles.gradeSummary}>
              <Text style={styles.gradeText}>Current Grade: {calculateGrade(item.components).toFixed(1)}%</Text>
            </View>
          </View>
        )}
      />
...
const styles = StyleSheet.create({
  ...
  addButton: {
    position: 'absolute', right: 20, bottom: 20, backgroundColor: '#2196F3', borderRadius: 30,
    width: 60, height: 60, justifyContent: 'center', alignItems: 'center', zIndex: 1,
  },
  courseCard: {
    backgroundColor: '#fff', borderRadius: 8, padding: 15, marginBottom: 10,
    shadowColor: '#000', shadowOpacity: 0.1, shadowOffset: { width: 0, height: 2 }, elevation: 5
  },
  ...
  componentItem: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#eee' },
  componentName: { flex: 2 },
  gradeSummary: { marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: '#ddd' },
  gradeText: { fontSize: 16, fontWeight: '600', color: '#2196F3' },
